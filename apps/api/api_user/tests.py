...
class ApiUserTest(APITestCase):
    ...
    def test_user_register(self):
        ...
        response = self.client.post('/api/api_user/register/', data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['user']['username'], 'TestUserRegister')

    def test_user_login(self):
        response = self.client.post('/api/api_user/login/', self.login_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_logout(self):
        ...
        response = self.client.post('/api/api_user/logout/', **header)
    ...
    def test_user_change_password(self):
        ...
        response = self.client.put('/api/api_user/change_password/', data, **header)
